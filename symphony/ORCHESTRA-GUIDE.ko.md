...
> 어디서든 보편 적용: 신규·초기·레거시·유지보수·운영 프로젝트 — SDD가 컨텍스트에 적응.
>
...
SDD는 하나의 흐름을 돌리되, 무엇을 발견하든 거기에 적응합니다:
...
흐름은 하나입니다. SDD가 작업 규모에 맞춰 조절하고(full / mini / none), 기존 코드가 있으면 컨텍스트 기반 단계를 더합니다 — 지휘자가 본 것에 반응하는 것뿐입니다.
...
> **\*0단계(Survey)**는 이해해야 할 기존 코드가 있을 때만 나타납니다. 신규 프로젝트는 조사할 게 없으니 SDD가 건너뜁니다. 컨텍스트가 부르면 같은 흐름에 조사와 회귀 검사가 포함될 뿐입니다.
...
매번 같은 SDD 흐름입니다. 다른 건 컨텍스트가 더하는 것뿐 — 기존 코드가 있으면 조사, 운영이면 더 엄격한 게이트.
...
> 보세요: 단계는 동일합니다. 운영이 다른 무언가를 발동한 게 아니라 — 강제 레벨을 올려 기존 게이트를 우회 불가로 만든 것뿐입니다.
...
같은 SDD 흐름이 프로젝트의 전 생애를 따라가며, 두 가지만 자동으로 변합니다:
...
톤 변경도, 이전도, CONSTITUTION 재작성도 없습니다. 프레임워크는 일정하고, 컨텍스트가 엄격도를 조절합니다 — 프로젝트가 성장해도 다시 설정할 일이 없습니다.
